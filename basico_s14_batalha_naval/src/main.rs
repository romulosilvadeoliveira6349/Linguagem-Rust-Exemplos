/*
	...
	- O aluno Christopher Krauss de Carvalho detectou um bug na versão original: No caso do 
	Windows cada tecla acionada era lida duplicada. Trata-se de uma limitação do crossterm::read,
	que lia duas vezes. O código foi alterado para considerar apenas quando a tecla é pressionada (Press).

    Event::Key(key_event) if key_event.kind == KeyEventKind::Press => {
        match (key_event.code, key_event.modifiers) {
		...

	Em https://docs.rs/crossterm/latest/crossterm/event/struct.KeyEvent.html#structfield.kind é dito:
		kind: KeyEventKind
			Kind of event.
			Only set if:
		...
		Mas no Ubuntu não foi necessário mudar flags. Caso necessário seria:
		stdout.execute( crossterm::event::PushKeyboardEnhancementFlags(crossterm::event::KeyboardEnhancementFlags::REPORT_EVENT_TYPES));
		... programa
		stdout.execute(crossterm::event::PopKeyboardEnhancementFlags);

		Inclusive no Ubuntu e no Windows
		println!("{:#?}", crossterm::terminal::supports_keyboard_enhancement());
		retorna falso.
*/
...
//				match (key_event.code, key_event.modifiers) {
			...
			Event::Key(_) => {},			// Quando Event::Key não for key_event.kind == KeyEventKind::Press
			Event::FocusGained => {},
			Event::FocusLost => {},
